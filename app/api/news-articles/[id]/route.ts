 ...
 * Single published row from the `articles` table (same as /api/articles/:id).
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    const { id } = await params;
    const { searchParams } = new URL(request.url);
    const query = parseQuery(searchParams, detailQuerySchema);
    ...
    console.error("GET /api/news-articles/[id]:", error);
    return jsonError("Failed to fetch news article", 500);
  }
}
