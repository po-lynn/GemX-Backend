/**
 * Google Cloud Translation API (v2) for title + body localization.
 * Supported locales: English, Myanmar, Thai, Korean.
 * Shared by News, Articles, and product description localization —
 * re-exported from features/news/services/google-translate.ts and
 * features/content/services/google-translate.ts to keep existing import paths.
 */
...
export async function buildLocalizedNews(
  ...
  if (!isGoogleTranslateConfigured()) {
    throw new Error(
      "Google Translate is not configured. Set GOOGLE_TRANSLATE_API_KEY to auto-translate this content.",
    )
  }
...
/** Map an edited locale's title/content onto the row columns for that locale. */
