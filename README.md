...
3. Optional: run migrations against Supabase from your machine:
   ...
   - If **`drizzle-kit` crashes** with `checkValue.replace` / `constraint_definition` (some Postgres/Supabase CHECK metadata), run **`npm install`** so **`patch-package`** applies `patches/drizzle-kit+0.31.8.patch`.
...
**Push notifications (optional):** Set server `FIREBASE_*` (Admin SDK) and `NEXT_PUBLIC_FIREBASE_*` + `NEXT_PUBLIC_FIREBASE_VAPID_KEY` for web push (see `.env.example`). The web app auto-subscribes to FCM topic `global` on load; publishing an article sends `articleId` + `articleTitle` to all subscribers. Mobile: subscribe to topic `global` on startup or use **POST /api/push/register** for user-targeted pushes. See [docs/guides/web-push-global-topic.md](docs/guides/web-push-global-topic.md) and [docs/MOBILE-API.md](docs/MOBILE-API.md).
