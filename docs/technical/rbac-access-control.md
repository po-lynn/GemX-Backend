...
- **`supervisor`** — configurable access. Each feature must be explicitly enabled for the supervisor role before it becomes accessible. All supervisors share the same permission profile (role-wide, not per-user).

Permissions are stored in the database and cached with Next.js tag-based invalidation. When an admin toggles access, the cache is busted immediately so the next supervisor request reflects the change.
...
Admin visits /admin/permissions
  ...
  → savePermissions() Server Action (app/admin/permissions/actions.ts)
      ...
      → setSupervisorPermissions(perms) (features/rbac/db/permissions.ts)
          → INSERT ... ON CONFLICT DO UPDATE for each feature key
          → revalidateTag("supervisor-permissions")
  → Next supervisor page request
      → requireFeatureAccess() or getSupervisorPermissions() called
      → unstable_cache miss (tag was invalidated) → fresh DB read
      → result cached under tag "supervisor-permissions"
```
...
| `drizzle/schema/rbac-schema.ts` | Drizzle table definition for `supervisor_permission` |
...
| `features/rbac/db/permissions.ts` | `getSupervisorPermissions()` (cached), `checkSupervisorAccess(key)`, `setSupervisorPermissions(perms)` |
...
| `app/admin/layout.tsx` | Fetches session + supervisor permissions, passes to sidebar |
...
Allows admins unconditionally; allows supervisors only when the feature is enabled.

| Condition | Result |
|-----------|--------|
| No session | redirect to `/login` |
| role is `admin` | returns session immediately (no DB check) |
| role is `supervisor`, feature enabled | returns session |
| role is `supervisor`, feature disabled | redirect to `/admin` |
...
| role is `supervisor`, feature enabled | `{ session }` |
| role is `supervisor`, feature disabled | `403 Forbidden` |
...
| Route | Reason |
|-------|--------|
| `GET/POST /api/admin/feature-settings` | App-wide feature flags; too sensitive to expose to supervisors |
...
`app/admin/layout.tsx` resolves the current session and, if the user is a supervisor, fetches the full permissions map from the cached `getSupervisorPermissions()`. Both `role` and `permissions` are passed as props to `AdminSidebar` (and `AdminSidebarSheet` for mobile).
...
Each `NavItem` in the nav config carries either `adminOnly: true` (permanently hidden from supervisors) or a `featureKey` (visible only when that key is enabled). Items with neither flag are visible to everyone.

**Design decision — role-wide permissions:** All supervisors share one permission profile. There is no per-user override mechanism. This mirrors Odoo's group-level access model and keeps permission management simple: one toggle affects all supervisor accounts simultaneously.

---

## Hardcoded Admin-Only Features

These features are intentionally excluded from `FEATURE_KEYS` and cannot be enabled for supervisors through the permissions UI:
...
| Authenticated but role is neither `admin` nor `supervisor` (e.g. `user`) | Middleware redirects to `/` before reaching any admin page |
| Supervisor visits a feature page that is disabled | `requireFeatureAccess()` → redirect `/admin` (dashboard) |
| Supervisor calls an API route for a disabled feature | `requireAdminOrFeature()` → `403 Forbidden` |
| Admin saves an empty permission set | `setSupervisorPermissions` skips the upsert but still calls `revalidateTag` |
