...
export function buildEditHref(
  ...
  return `/portal/products/${id}/edit?${p.toString()}`
}
...
export function PortalProductsListView({
  products,
  views,
  activeView,
  page,
    ...
    <ListViewCard
      ...
      getGroupKey={(r, grp) => {
        switch (grp) {
          ...
          case "review":     return STATUS_LABELS[r.moderationStatus] ?? r.moderationStatus
          case "visibility": {
            const parts: string[] = []
      ...
      filterRow={(r, filterId, vals) => {
        switch (filterId) {
          ...
          case "review":     return vals.includes(r.moderationStatus)
          case "flags":
            return vals.every((f) =>
