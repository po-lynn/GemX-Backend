/**
 * Google Cloud Translation API (v2) for content title + body localization.
 * Supported locales: English, Myanmar, Thai, Korean.
 * Shared by Articles (and product description localization).
 */
...
export async function buildLocalizedNews(
  ...
  if (!isGoogleTranslateConfigured()) {
    throw new Error(
      "Google Translate is not configured. Set GOOGLE_TRANSLATE_API_KEY to auto-translate content.",
    )
  }
...
/** Map an edited locale's title/content onto the content row columns. */
