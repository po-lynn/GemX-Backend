...
const PLACEHOLDER_GRADIENTS = [
  ...
  "linear-gradient(150deg,#dbe7ff,#9fb9ee)",
]
...
export function FeaturedSection({ products }: Props) {
  ...
  return (
    <section id="marketplace" style={{ padding: "84px 7vw 0" }}>
      <div style={{
      ...
      <div className="home-featured-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
        {display.length > 0 ? display.map((item, i) => {
            ...
            >
              <div style={{
                ...
                <span style={{
                  ...
                  color: badge.color, background: "rgba(255,255,255,0.95)", padding: "4px 10px",
          ...
          /* Placeholder skeleton when no featured products yet */
          [0,1,2].map((i) => (
            <div key={i} style={{
              border: "1px solid #ededf2", borderRadius: 18, overflow: "hidden",
