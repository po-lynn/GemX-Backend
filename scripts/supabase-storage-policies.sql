...
-- Create buckets in Dashboard > Storage: product-images, product-videos, product-certificates, user-images (all public if you want direct links).
...
DROP POLICY IF EXISTS "Service role full user-images" ON storage.objects;
...
-- Allow service_role full access to user-images bucket (profile/avatar images)
CREATE POLICY "Service role full user-images"
ON storage.objects
FOR ALL
TO service_role
